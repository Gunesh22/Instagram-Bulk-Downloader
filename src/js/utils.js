...
async function saveZip() {
    ...
    async function fetchSelectedMedia() {
        let count = 0;
        const promises = media.map(async (media) => {
            const files = [];
            ...
            try {
                ...
                const baseTitle = media.title.replaceAll(' | ', '_');
                files.push({
                    title: media.nodeName === 'VIDEO' ? `${baseTitle}.mp4` : `${baseTitle}.jpeg`,
                    data: blob
                });

                // 2. Caption
                if (media.dataset.caption) {
                    files.push({
                        title: `${baseTitle}.txt`,
                        data: new Blob([media.dataset.caption], { type: 'text/plain' })
                    });
                }

                // 3. Thumbnail (for video)
                if (media.dataset.thumbnail && media.nodeName === 'VIDEO') {
                    try {
                        const tRes = await fetch(media.dataset.thumbnail);
                        const tBlob = await tRes.blob();
                        files.push({
                            title: `${baseTitle}_thumb.jpg`,
                            data: tBlob
                        });
                    } catch (err) {
                        console.log('Thumbnail fetch fail', err);
                    }
                }

            } catch (e) {
                console.log('Media fetch fail', e);
                throw e;
            }
        ...
        return results.map(promise => promise.value).flat();
    }
    try {
...
function renderMedia(data) {
    ...
    // TITLE_CONTAINER.classList.remove('multi-select'); // Keep selection mode if active (handled by caller)
